...
def test_double_slash_rejected():
    """owner//repo is rejected (repeated slash)."""
    dest = "/tmp/test_downloads"
    with pytest.raises(ValueError, match="repeated slash"):
        workspace_path("owner//repo", dest)


def test_dot_segment_rejected():
    """owner/./repo is rejected (dot segment)."""
    dest = "/tmp/test_downloads"
...
def test_trailing_slash_rejected():
    """owner/repo/ is rejected (trailing slash)."""
    dest = "/tmp/test_downloads"
    with pytest.raises(ValueError, match="leading or trailing slash"):
        workspace_path("owner/repo/", dest)
...
def test_leading_slash_rejected():
    """/owner/repo is rejected (leading slash)."""
    dest = "/tmp/test_downloads"
    with pytest.raises(ValueError, match="leading or trailing slash"):
        workspace_path("/owner/repo", dest)


def test_three_segments_rejected():
    """owner/repo/extra is rejected (more than two segments)."""
    dest = "/tmp/test_downloads"
    with pytest.raises(ValueError, match="exactly two path segments"):
        workspace_path("owner/repo/extra", dest)


def test_empty_namespace_rejected():
    """/repo (empty namespace) is rejected."""
    dest = "/tmp/test_downloads"
    with pytest.raises(ValueError):
        workspace_path("/repo", dest)


def test_empty_repository_rejected():
    """owner/ (empty repository) is rejected."""
    dest = "/tmp/test_downloads"
    with pytest.raises(ValueError):
        workspace_path("owner/", dest)


def test_traversal_with_slash_rejected():
    """owner/../repo is rejected (path traversal)."""
    dest = "/tmp/test_downloads"
    with pytest.raises(ValueError, match="path traversal"):
        workspace_path("owner/../repo", dest)
